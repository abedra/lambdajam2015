...
unsigned char *encrypt(char key[DES_KEY_SIZE], char msg[8])
{
  ...
  strncpy(plain_text, msg, 8);
  plain_text[8] = '\0';

  unsigned char k[DES_KEY_SIZE];
  strncpy(k, key, DES_KEY_SIZE);
  k[8] = '\0';

  unsigned char *cipher_text = malloc(8);

  des_setkey_enc(&ctx, k);
  des_crypt_ecb(&ctx, plain_text, cipher_text);
  cipher_text[8] = '\0';
...
unsigned char *decrypt(char key[DES_KEY_SIZE], unsigned char msg[8])
{
  ...
  unsigned char k[DES_KEY_SIZE];
  memcpy(key, k, DES_KEY_SIZE);
  k[8] = '\0';

  unsigned char cipher_text[8];
  memcpy(msg, cipher_text, 8);
  cipher_text[8] = '\0';

  unsigned char *plain_text = malloc(8);

  des_setkey_dec(&ctx, k);
  des_crypt_ecb(&ctx, cipher_text, plain_text);
  plain_text[8] = '\0';
